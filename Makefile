...
run-tests-fastest-php: ## Runs all test via fastest (pure PHP)
	...
	@bin/console cache:clear --env=test
	@find tests/ -name "*Test.php" | php ./vendor/bin/fastest -v -p 8 -o "php ./vendor/bin/phpunit {} -c phpunit.fastest.xml --coverage-php build/fastest/{n}.cov --log-junit build/fastest/{n}.xml";
