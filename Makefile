...
run-tests-fastest-php: ## Runs all test via fastest (pure PHP)
	...
	@bin/console cache:clear --env=test
	ifeq ($(TRAVIS), "true")
		@find tests/ -name "*Test.php" | travis_wait php ./vendor/bin/fastest -v -p 8 -b "php ./tests/bootstrap.php" "php ./vendor/bin/phpunit {} -c phpunit.fastest.xml --coverage-php build/fastest/{n}.cov --log-junit build/fastest/{n}.xml";
	else
...
run-tests-fastest-phpdbg: ## Runs all test via fastest (phpdbg)
	...
	@mkdir -p build/fastest
	@bin/console cache:clear --env=test
	@find tests/ -name "*Test.php" | php ./vendor/bin/fastest -v -p 8 -b "php ./tests/bootstrap.php" "phpdbg -qrr -d memory_limit=4096M ./vendor/bin/phpunit {} -c phpunit.fastest.xml --coverage-php build/fastest/{n}.cov --log-junit build/fastest/{n}.xml";
