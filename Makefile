...
run-tests-fastest-phpdbg: ## Runs all test via fastest (phpdbg)
	...
	@mkdir -p build/fastest
	@bin/console cache:clear --env=test
	@ulimit -n 1000000 find tests/ -name "*Test.php" | php ./vendor/bin/fastest -v -p 8 -b "php ./tests/bootstrap.php" "phpdbg -qrr -d memory_limit=-1 ./vendor/bin/phpunit {} -c phpunit.fastest.xml --coverage-php build/fastest/{n}.cov --log-junit build/fastest/{n}.xml";
